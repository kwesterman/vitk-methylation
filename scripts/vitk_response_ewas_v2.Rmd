...
run_baseline_model <- function(probeData, covarData, formula_string) {
  ...
  tryCatch({
    lm_fit <- lm(as.formula(formula_string), data=regData)
    # sandwich_fit <- coeftest(lm_fit, vcov=vcovHC(lm_fit, type="HC"))
    # coefs <- sandwich_fit["lnBK", c("Estimate", "t value", "Pr(>|t|)")]
...
```{r baseline-outputs}
#manhattan(baseline_res_df, chr="chr", bp="pos", p="p")

lambda_baseline <- gControl(baseline_res_df$p)
make_qqplot(baseline_res_df$p, 
            plotTitle=paste0("lambda: ", round(lambda_baseline, 2)))

# top_cpgs <- baseline_res_df$CpG[1:4]
# bind_rows(list(model1=filter(baseline_res_df, CpG %in% top_cpgs),
#                model2=filter(baseline_model2_res_df, CpG %in% top_cpgs),
#                model3=filter(baseline_model3_res_df, CpG %in% top_cpgs)),
#           .id="model") %>% 
#   mutate(Gene=gsub(";.*", "", UCSC_RefGene_Name),
#          res_string=paste0(round(beta, 2), " (", 
#                            format(p, scientific=T, digits=3), ")")) %>%
...
# Manuscript tables/figures skeleton
...
# lm1 <- lm(ilogit2(mvals[responder_res_tbl$CpG[1], ]) ~ responder, data=nmd)
# lm2 <- lm(ilogit2(mvals[responder_res_tbl$CpG[2], ]) ~ responder, data=nmd)
...
nominal_overlaps <- combp_res_responder %>%
  ...
  # filter(p_region_slk < 0.01,
  #        p_region_slk_baseline < 0.01) %>%
...
#   select(chr, pos, num_cpgs, p_region_slk, p_adj_region_sidak) %>%
#   setNames(c("Chromosome", "Position", "# CpGs", "SLK p-value", "Adjusted p-value"))
# 
# combp_res_responder %>%
#   filter(p_region_slk < 1e-5) %>%
#   mutate(chr=gsub("chr", "", chr),
...
# lipid_related_genes <- c("NPC1L1", "APOA4", "APOA5", "APOE", "SORT1", "LPL")
lipid_related_genes <- c("NPC1L1", "APOA1", "APOA5", "APOE", "LPL", "APOB")
# lipid_related_genes <- c("NPC1L1", "APOA4", "APOA5", "APOE", "SORT1", "LPL", "APOB", "APOA1", "APOA2", "APOC3")
...
coMET::comet.web(
  ...
  # config.file="../int/npc1l1_config.txt"
...
# cor_obj <- cor.test(~beta.responder+beta.baseline, 
#                     data=filter(double_res, p.responder < 0.05, p.baseline < 0.05))
# 
...
#   data=filter(double_res_model3, p.responder < 0.05, p.baseline < 0.05))
# 
# double_res_full_models <- inner_join(responder_model3_res_df, baseline_model4_res_df,
#                                      by=c("CpG","chr","pos","UCSC_RefGene_Name"),
#                                      suffix=c(".responder",".baseline"))
# cor_obj_full_models <- cor.test(
#   ~beta.responder+beta.baseline,
#   data=double_res_full_models)
# cor_obj_full_models_nominal <- cor.test(
#   ~beta.responder+beta.baseline,
#   data=filter(double_res_full_models, p.responder < 0.05, p.baseline < 0.05))
# 
# cor_plt_df <- filter(double_res_full_models, p.responder < 0.05 & p.baseline < 0.05)
...
#   # annotate("text", label=paste0("Correlation of EWAS coefficients: ",
#   #                               round(cor_obj_full_models_nominal$estimate, 2),
#   #                               " (p=", signif(cor_obj_full_models_nominal$p.value, 2), ")"),
#   #          x=-0.3, y=-0.3) +
#   annotate("text", x=-0.3, y=-0.3, label=expression(rho*"=0.86 (p~0)")) +
#   labs(
#     # x="Responder effect size (arbitrary units)",
...
#     # y="Baseline effect size (arbitrary units)"
#   ) +
#   theme_bw()
```
...
ggplot(filter(manhattan_plt_df, abs(negLogP) > 0), aes(x=pos_cum, y=negLogP)) +
  ...
  geom_hline(yintercept=c(-5, 5), color="blue") +
  geom_hline(yintercept=c(-1, 1) * -log10(bonferroni_level), color="red") +
  scale_color_manual(values=rep(c("gray", "turquoise3"), 22)) +
...
ggplot(cor_plt_df, aes(x=beta.responder, y=beta.baseline)) +
  ...
  #                               " (p=", signif(cor_obj_full_models_nominal$p.value, 2), ")"),
  #          x=-0.3, y=-0.3) +
  annotate("text", x=0, y=0.23, label=expression(rho*"=0.38 (p=1.7e-15)")) +
  annotate("text", x=0, y=0.2, label="Fraction sharing sign = 0.96") +
  labs(
    # x="Responder effect size (arbitrary units)",
    x=expression(Delta*"M-value between non-responders and responders"),
    y=expression(Delta*"M-value per unit log(plasma phylloquinone)")
...
# double_res_adj <- inner_join(responder_model3_res_df, baseline_model4_res_df,
#                          by=c("CpG","chr","pos","UCSC_RefGene_Name"),
#                          suffix=c(".responder",".baseline"))
