...
```{r responder-ewas-unadj, cache=1}
responder_formula_unadj <- "meth ~ responder"

cl <- makePSOCKcluster(32)
registerDoParallel(cl)

responder_ewas_unadj_res_list <- foreach(
  meth=iter(mvals, by="row")) %dopar%
  run_responder_model(meth, nmd, responder_formula_unadj)

stopCluster(cl)
```

```{r responder-ewas-unadj-processing}
responder_unadj_res_df <- do.call(rbind, responder_ewas_unadj_res_list) %>%
  data.frame(stringsAsFactors=F) %>%
  mutate_at(c("beta", "t", "p"), as.numeric) %>%
  inner_join(dplyr::select(epic_anno, Name, chr, pos, UCSC_RefGene_Name), 
             by=c("CpG"="Name")) %>%
...
```{r responder-outputs}
lambda_responder_unadj <- gControl(responder_unadj_res_df$p)
lambda_responder <- gControl(responder_res_df$p)
lambda_responder_model2 <- gControl(responder_model2_res_df$p)
lambda_responder_model3 <- gControl(responder_model3_res_df$p)
make_qqplot(responder_res_df$p, 
            plotTitle=paste0("lambda: ", round(lambda_responder, 2)))
```

```{r pheno-followup}
a <- select(nmd, contains("cpPC"), age, M1F0, CD4T, CD8T, NK, Bcell, Mono, lnTG)
a <- as.matrix(a)
cor_a <- cor(a)
cor_a[abs(cor_a) < 0.4] <- 0

cpacor_var_expl <- cumsum(pca.fit$values) / sum(pca.fit$values)

# We examined correlations between these technical principal components and both biological variables (age/sex/cell composition) and technical variables (array and chip row), finding that CPACOR PCs 2&3 are strongly associated with sex (corr. > 0.5), while other PCs are associated with CD4T (PC8) or CD8T (PCs 4&7) (corrs. ~0.3-0.4). 

for(pc in paste0("cpPC", 1:10)) {
  print(broom::tidy(lm(as.formula(paste0(pc, "~Sentrix_Position")), data=nmd)))
}
for(pc in paste0("cpPC", 1:10)) {
  print(broom::tidy(lm(as.formula(paste0(pc, "~as.character(Sentrix_ID)")), data=nmd)))
}
```
...
# responder_res_tbl$beta_diff <- c(lm1$coefficients["responderTRUE"],  # Corresponds to "adjusted" beta value differences
#                                  lm2$coefficients["responderTRUE"])

responder_res_tbl$beta_diff <- c(  # Corresponds to raw beta value differences
  summary(lm(ilogit2(
    mvals[responder_res_tbl$CpG[1], ]) ~ nmd$responder))$coef[2, 1],
  summary(lm(ilogit2(
    mvals[responder_res_tbl$CpG[2], ]) ~ nmd$responder))$coef[2, 1]
)
...
responder_model2_res_df %>%
  mutate(chr=paste0("chr", chr),
         start=pos - 1,
         end=pos) %>%
  arrange(chr, pos) %>%
  select(chrom=chr, chromStart=start, chromEnd=end, p=p, name=CpG) %>%
  write_tsv("../int/combp/responder_model2_res.bed")

baseline_model2_res_df %>%
  mutate(chr=paste0("chr", chr),
         start=pos - 1,
         end=pos) %>%
  arrange(chr, pos) %>%
  select(chrom=chr, chromStart=start, chromEnd=end, p=p, name=CpG) %>%
  write_tsv("../int/combp/baseline_model2_res.bed")
```
...
combp_res_responder_model2 <- read_tsv("../int/combp/regions.sig_responder_model2_Res.bed", col_names=F) %>%
  setNames(c("chr", "start", "end", "lowest_p", "num_cpgs", 
             "p_region_slk", "p_adj_region_sidak")) %>%
  arrange(p_adj_region_sidak) %>%
  filter(p_region_slk < 0.05, num_cpgs >= 2)
combp_ranges_responder_model2 <- GRanges(combp_res_responder_model2$chr, 
                                  IRanges(combp_res_responder_model2$start, combp_res_responder_model2$end))
...
combp_res_baseline_model2 <- read_tsv("../int/combp/regions.sig_baseline_model2_res.bed", col_names=F) %>%
  setNames(c("chr", "start", "end", "lowest_p", "num_cpgs", 
             "p_region_slk", "p_adj_region_sidak")) %>%
  arrange(p_adj_region_sidak) %>%
  filter(p_region_slk < 0.05, num_cpgs >= 2)
combp_ranges_baseline_model2 <- GRanges(combp_res_baseline_model2$chr, 
                                  IRanges(combp_res_baseline_model2$start, combp_res_baseline_model2$end))

# Overlap between responder and baseline DMRs
...
# combp_res_responder %>%
#   filter(p_adj_region_sidak < 0.01) %>%
...
responder_models_overlap_ranges <- findOverlaps(combp_ranges_responder, combp_ranges_responder_model2)
responder_model2_overlaps <- combp_res_responder_model2 %>%
  dplyr::slice(to(responder_models_overlap_ranges)) %>%
  select(chr, start, end, p_adj_region_sidak_model2=p_adj_region_sidak)

combp_res_responder %>%
  filter(p_adj_region_sidak < 0.05) %>%
  left_join(responder_model2_overlaps, by=c("chr", "start", "end")) %>% 
  mutate(chr=gsub("chr", "", chr)) %>%
  mutate(annot_gene=find_genes_by_region(.),
         chr=paste0("chr", chr),
         loc=paste0(chr, ":", start, "-", end)) %>%
  ...
  mutate(annot_gene=unlist(lapply(strsplit(annot_gene, split=";"), function(x) paste(unique(x), collapse="; ")))) %>%
  select(loc, num_cpgs, annot_gene, 
         p_region_slk, contains("p_adj_region")) %>%
  setNames(c("Locus", "# CpGs", "Annotated gene", "P", "Adj. P", "Adj. P (model 2)")) %>%
  write_csv("../output/combp_responder_with_model2_res_tbl.csv")
...
baseline_models_overlap_ranges <- findOverlaps(combp_ranges_baseline, combp_ranges_baseline_model2)
baseline_model2_overlaps <- combp_res_baseline_model2 %>%
  dplyr::slice(to(baseline_models_overlap_ranges)) %>%
  select(chr, start, end, p_adj_region_sidak_model2=p_adj_region_sidak)

combp_res_baseline %>%
  filter(p_adj_region_sidak < 0.05) %>%
  left_join(baseline_model2_overlaps, by=c("chr", "start", "end")) %>% 
  mutate(chr=gsub("chr", "", chr)) %>%
  mutate(annot_gene=find_genes_by_region(.),
         chr=paste0("chr", chr),
         loc=paste0(chr, ":", start, "-", end)) %>%
  ...
  select(loc, num_cpgs, annot_gene, 
         p_region_slk, contains("p_adj_region")) %>%
  setNames(c("Locus", "# CpGs", "Annotated gene", "P", "Adj. P", "Adj. P (model 2)")) %>%
  write_csv("../output/combp_baseline_with_model2_res_tbl.csv")
```

```{r responder-lipid-top-hits}
...
snp_data <- read_csv("../data/NPC1L1_p_ucOC.csv") %>%
  select(hnrcid, rs2072183_add)
nmd_with_npc1l1_snp <- nmd %>%
  left_join(snp_data, by="hnrcid") %>%
  mutate(meth=mvals[lipid_related_tbl$CpG[1], ])
npc1l1_snp_only_lm <- lm(as.formula(paste0("meth ~ rs2072183_add + ", paste0("cpPC", 1:10, collapse=" + "))),
                         data=nmd_with_npc1l1_snp)
npc1l1_snp_only_lm_model2 <- lm(as.formula(paste0("meth ~ rs2072183_add + sex + age + CD4T + CD8T + NK + Bcell + Mono + ", 
                                                  paste0("cpPC", 1:10, collapse=" + "))),
                                data=nmd_with_npc1l1_snp)
npc1l1_with_snp_lm <- lm(as.formula(paste0(responder_formula, " + rs2072183_add")),
                         data=nmd_with_npc1l1_snp) 
npc1l1_with_snp_lm_model2 <- lm(as.formula(paste0(responder_formula_model2, " + rs2072183_add")),
                                data=nmd_with_npc1l1_snp)
...
```{r alternate-status-measures}
top_responder_cpgs <- responder_res_df$CpG[1:10]

run_Kstatus_model <- function(probeData, covarData, formula_string, status_measure) {
  CpG <- rownames(probeData)
  regData <- cbind(meth=as.vector(probeData), covarData)
  ...
  outlier_TF <- (regData$meth < quantile(regData$meth, 0.25) - 3 * IQR(regData$meth) | 
                   (regData$meth > quantile(regData$meth, 0.75) + 3 * IQR(regData$meth)))
  regData <- regData[!outlier_TF, ]
  formula_string <- gsub("responder", status_measure, formula_string)

  tryCatch({
    lm_fit <- lm(as.formula(formula_string), data=regData)
    coefs <- summary(lm_fit)$coef[status_measure, c("Estimate", "t value", "Pr(>|t|)")]
    setNames(c(CpG, coefs), c("CpG", "beta", "t", "p"))
  }, error=function(e) rep(NA, 3))
}

nmd_with_status <- read_csv("../data/NPC1L1_p_ucOC.csv") %>%
  mutate(median_p_ucOC=rowMedians(as.matrix(.[, 5:8]), na.rm=T),
         delta_p_ucOC=median_p_ucOC - p_ucOC_0) %>%
  inner_join(nmd, by="hnrcid")

p_ucOC_ewas_res_list <- foreach(
  meth=iter(mvals[top_responder_cpgs, ], by="row")) %do%
  run_Kstatus_model(meth, nmd_with_status, responder_formula, "delta_p_ucOC")
p_ucOC_res_df <- do.call(bind_rows, p_ucOC_ewas_res_list)

lipid_p_ucOC_ewas_res_list <- foreach(
  meth=iter(mvals[rownames(mvals) %in% lipid_related_cpgs, ], by="row")) %do%
  run_Kstatus_model(meth, nmd_with_status, responder_formula, "delta_p_ucOC")
lipid_p_ucOC_res_df <- do.call(bind_rows, lipid_p_ucOC_ewas_res_list)

p_ucOC_responder_tt <- t.test(delta_p_ucOC ~ responder, data=nmd_with_status)
```
...
double_res_fa <- inner_join(responder_model3_res_df, baseline_model4_res_df,  # fa = fully adjusted
                         by=c("CpG","chr","pos","UCSC_RefGene_Name"),
                         suffix=c(".responder",".baseline"))
...
# Code based on https://www.r-graph-gallery.com/wp-content/uploads/2018/02/Manhattan_plot_in_R.html
...
with(cor_plt_df, {
  same_sign_enrich <- missMethyl::gometh(CpG[sign(beta.responder) == sign(beta.baseline)],
    ...
    arrange(P.DE)
  write_csv(same_sign_enrich, "../output/gsa_same_sign.csv")
  diff_sign_enrich <- missMethyl::gometh(CpG[sign(beta.responder) != sign(beta.baseline)],
                                         all.cpg=CpG, array.type="EPIC") %>%
    arrange(P.DE)
  write_csv(diff_sign_enrich, "../output/gsa_diff_sign.csv")
})       

same_sign_cpgs <- cor_plt_df$CpG[sign(cor_plt_df$beta.responder) == sign(cor_plt_df$beta.baseline)]
same_sign_genes <- epic_anno %>%
  filter(Name %in% same_sign_cpgs) %>%
  select(CpG=Name, Gene=UCSC_RefGene_Name) %>%
  separate_rows(Gene, sep=";") %>%
  distinct(Gene)
write_csv(same_sign_genes, "../output/same_sign_genes.csv")

diff_sign_cpgs <- cor_plt_df$CpG[sign(cor_plt_df$beta.responder) != sign(cor_plt_df$beta.baseline)]
diff_sign_genes <- epic_anno %>%
  filter(Name %in% diff_sign_cpgs) %>%
  select(CpG=Name, Gene=UCSC_RefGene_Name) %>%
  separate_rows(Gene, sep=";") %>%
  distinct(Gene)
write_csv(diff_sign_genes, "../output/diff_sign_genes.csv")
```
...
# For retrieving raw fractional methylation differences
ncor2_tt <- t.test(ilogit2(mvals["cg10918016", ]) ~ nmd$responder)
xpc_tt <- t.test(ilogit2(mvals["cg04894169", ]) ~ nmd$responder)
```

```{r data-for-deposit}
nmd %>%
  mutate(age=round(age)) %>%
  select(sampleKey, responder, baseline_K, sex, age) %>%
  write_csv("../output/metadata.csv")

write.table(betas, "../output/beta_matrix.txt")
```
